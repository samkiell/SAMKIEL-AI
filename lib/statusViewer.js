...
async function handleAutoStatus(sock, message) {
  ...
  let config = {
    enabled: settings.featureToggles.AUTO_STATUS_VIEW !== "off",
    reactOn: settings.featureToggles.ENABLE_STATUS_REACTION ?? false,
    emoji: settings.featureToggles.STATUS_VIEW_EMOJI || "👀",
    msgEnabled: settings.featureToggles.STATUS_VIEW_MSG === "on",
  ...
  try {
    if (fs.existsSync(configPath)) {
      ...
      // Initialize with settings.js values if file doesn't exist
  ...
  try {
    ...
    // 4. Handle Download mode - DISABLED per user request to save space
    // Only manual download via .savestatus command is allowed now
    /*
