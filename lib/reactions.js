...
function loadAutoReactionState() {
  ...
  try {
    if (fs.existsSync(USER_GROUP_DATA)) {
      ...
      if (data.autoReaction !== undefined) return data.autoReaction;
    }
  } catch (error) {
    console.error("Error loading auto-reaction state:", error);
  }
  return settings.featureToggles.AUTO_REACTION ?? false;
}
...
async function handleAreactCommand(sock, chatId, message, isOwner) {
  try {
    ...
    if (action === "on") {
      ...
      await sock.sendMessage(chatId, {
        text: `Auto-reactions are currently ${currentState} globally.\n\nUse:\n${p}areact on - Enable auto-reactions\n${p}areact off - Disable auto-reactions`,
        quoted: message,
      });
    }
  } catch (error) {
...
// Helper to check if auto-react is currently enabled
function isAutoReactOn() {
  return isAutoReactionEnabled;
}

// Helper function for commands to send reactions while respecting the toggle
