...
function isGlobalEnabled() {
  return state.global?.enabled !== false;
}

function setGlobalEnabled(enabled, enabledBy = null) {
  if (!state.global) state.global = { enabled: true };
  state.global.enabled = enabled;
  state.global.disabledBy = enabled ? null : enabledBy;
  saveState();
}
