...
function loadPrefix() {
  ...
  try {
    if (fs.existsSync(PREFIX_FILE)) {
      ...
      // Force sync: if stored prefix differs from settings.js, update it
      // This ensures that a deployment with a specific settings.js configuration overrides old state
      if (data.prefix !== settingsPrefix) {
        savePrefix(settingsPrefix);
    ...
    // File invalid, force save default from settings
    savePrefix(settingsPrefix);
    return settingsPrefix;
  }
}
