...
function normalizeToNumber(id) {
  ...
  const strId = typeof id === "object" ? id?.number || id?.jid || id?.id || "" : String(id);
  return strId
...
function loadOwnerData() {
  try {
    if (fs.existsSync(ownerPath)) {
      ...
      // Ensure structure
      if (!data.superOwner) data.superOwner = [normalizeToNumber(settings.ownerNumber)];
...
async function resolveAndUpdateOwnerLid(sock, senderId) {
  ...
  try {
    ...
    if (!alreadyHasLid && (ownerIndex !== -1 || isSuperOwnerMatch)) {
      ...
      const targetJid = senderId.includes("@") ? senderId : senderNum + "@s.whatsapp.net";
      ...
      if (result && result.length > 0 && result[0].lid) {
        ...
        if (ownerIndex !== -1) {
          ...
          const alreadyInList = ownersList.some(o => normalizeToNumber(o?.lid) === resolvedLid);
          if (!alreadyInList) {
...
function isSuperOwner(senderId) {
  try {
    ...
    const superOwners = Array.isArray(data.superOwner) ? data.superOwner : [data.superOwner];
...
async function isOwner(senderId, sock = null, messageKey = null) {
  try {
    ...
    const settingsOwner = normalizeToNumber(settings.ownerNumber || "2348087357158");
    ...
    const isMatched = ownersList.some((owner) => {
      const num = normalizeToNumber(typeof owner === "object" ? owner.number : owner);
      const lid = typeof owner === "object" ? normalizeToNumber(owner.lid || "") : "";
