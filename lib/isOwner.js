 ...
 * @param {object} sock - Socket for LID resolution (optional)
 * @param {object} messageKey - Message key object for fromMe check (optional)
 */
async function isOwner(senderId, sock = null, messageKey = null) {
  try {
    // Quick fromMe check - if message is from the bot itself, it's owner
...
/**
 * Synchronous owner check (no LID resolution, but faster)
 * Use for quick checks where async isn't needed
 */
